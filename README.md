...
Both hands are **centred on their own staff** — the right on the treble, the left on the bass — by shifting
each line as a whole by octaves. Built relative to the tonic and left alone, the texture rides up with the
key: B major sat four ledger lines above the bass staff and five above the treble, with both staves empty
underneath. Shifting by whole octaves, and the whole line at once, changes no interval and no voice leading,
only the clef it is written against. A one-octave run has to leave its staff somewhere, so two ledger lines
is the floor rather than zero, and a test holds it there in all 24 keys.
